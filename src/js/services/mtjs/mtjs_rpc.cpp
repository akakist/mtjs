...
bool MTJS::Service::mtjsRpcREQ(const mtjsEvent::mtjsRpcREQ *e, const REF_getter<epoll_socket_info> &esi)
{
    ...
    JSValueGuard global(js_ctx, JS_GetGlobalObject(js_ctx));
    auto ret = JS_Call(js_ctx, it->second.get(), global.get(), 1, &jsreq);
    scope.addValue(ret);
...
bool MTJS::Service::mtjsRpcRSP(const mtjsEvent::mtjsRpcRSP *e, const REF_getter<epoll_socket_info> &esi)
{
    ...
    JSValueGuard global(js_ctx, JS_GetGlobalObject(js_ctx));
    auto ret = JS_Call(js_ctx, it->second.get(), global.get(), 1, &jsrsp);
    scope.addValue(ret);
