...
std::string base16::encode(const std::vector<uint8_t> &input)
{
    return encode(input.data(), input.size());
}
std::string base16::encode(const uint8_t* data, size_t length)
{
...
std::string base16::decode(const std::vector<uint8_t>& data)
{
    return decode((const uint8_t*)data.data(), data.size());
}
...
int main() {
    ...
    auto enc=base16::encode((const uint8_t*)"Sergey Belyalov", 15);
    std::cout<<"base16: "<<enc<<std::endl;
    auto dec=base16::decode((const uint8_t*)enc.data(), enc.size());
