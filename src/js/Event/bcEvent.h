...
class TxValidatorStart: public Event::NoPacked
   ...
   class SendToChildAck: public Event::Base
    ...
    #ifdef KALL
   class HeartBeatREQ: public NetworkBase
    {

    public:
        static Base* construct(const route_t &r)
        {
            return new HeartBeatREQ(r);
        }
        HeartBeatREQ(const BLOCK_id& _prev_block_hash, 
            const BigInt _epoch,
            const NODE_id& _node_leader,
            const route_t& r)
            :NetworkBase(bcEventEnum::HeartBeatREQ,r), 
            prev_block_hash(_prev_block_hash),
            epoch(_epoch),
            node_leader(_node_leader)   {}
        
        BLOCK_id prev_block_hash;
        BigInt epoch;
        NODE_id node_leader;

        void  hash(Blake2bHasher &h) const
        {
        ...
        HeartBeatREQ(const route_t& r)
            :NetworkBase(bcEventEnum::HeartBeatREQ,r) {}

        void unpack(inBuffer& o)
        {   

            o>>prev_block_hash>>epoch>>node_leader;
        }
        void pack(outBuffer& o) const
        {
