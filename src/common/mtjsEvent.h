...
{
    const SERVICE_id mtjs(genum_mtjs);
...
namespace mtjsEventEnum
{
    ...
    const EVENT_id AsyncExecuted(genum_mtjs_async_executed);
    const EVENT_id Eval(genum_mtjs_Eval);
    const EVENT_id EmitterData(genum_EmitterData);
    const EVENT_id mtjsRpcREQ(genum_mtjsRpcREQ);
    const EVENT_id mtjsRpcRSP(genum_mtjsRpcRSP);
}
